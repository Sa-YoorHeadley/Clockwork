...
export default function SearchBar({setSearchKey, setSearchOption, dataKeys, listType, listStatus}) {
    ...
    const [filteredKeys, setFilteredKeys] = useState(['Name', 'Location', 'ID', 'Email', 'Phone Number', 'Status', 'Other'])
    ...
    return (
      <div className= "filter function">
        ...
        <div className='filter-text'>
            <input type="text" placeholder='Filter current results' className='filter-text-input' onChange={(event)=>{setSearchKey(event.target.value)}} />
          ...
          <select defaultValue={'ID'} onChange={handleChange} className='filter-select'>
              <option value='ID' disabled hidden>ID</option>
